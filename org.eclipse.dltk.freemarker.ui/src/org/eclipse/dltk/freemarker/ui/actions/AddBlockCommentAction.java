...
/**
 * Action that encloses the editor's current selection with FTL block comment
...
public class AddBlockCommentAction extends BlockCommentAction {
	...
	protected void runInternal(ITextSelection selection,
		...
		ITypedRegion partition = docExtension.getPartition(
				IFreemarkerPartitions.FREEMARKER_PARTITIONING, selectionOffset,
				false);
	 ...
	 *            <code>partition</code>
	 */
	private void handleFirstPartition(ITypedRegion partition, List<Edit> edits,
		...
		if (partType == IDocument.DEFAULT_CONTENT_TYPE
				|| partType == IFreemarkerPartitions.FTL_ASSIGN
				|| partType == IFreemarkerPartitions.FTL_INCLUDE
				|| partType == IFreemarkerPartitions.FTL_IF_DIRECTIVE_START
				|| partType == IFreemarkerPartitions.FTL_LIST_DIRECTIVE_START
				|| partType == IFreemarkerPartitions.FTL_INTERPOLATION) {
	...
	private ITypedRegion handleInteriorPartition(ITypedRegion partition,
			List<Edit> edits, Edit.EditFactory factory,
		...
		// if (partType == IFreemarkerPartitions.FREEMARKER_DOC) {

		// wasJavadoc= true;

		// } else
		if (partType == IFreemarkerPartitions.FREEMARKER_COMMENT) {
		...
		partition = docExtension.getPartition(
				IFreemarkerPartitions.FREEMARKER_PARTITIONING, partEndOffset,
				false);
		...
		if (wasJavadoc) {
			...
			// if (partType == IJavaScriptPartitions.FREEMARKER_DOC) {
	 ...
	 */
	private void handleLastPartition(ITypedRegion partition, List<Edit> edits,
	...
	private boolean isSpecialPartition(String partType) {
		return partType == IFreemarkerPartitions.FREEMARKER_STRING;
