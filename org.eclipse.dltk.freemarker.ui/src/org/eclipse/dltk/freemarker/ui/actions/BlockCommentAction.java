...
public abstract class BlockCommentAction extends TextEditorAction {
	 ...
	 * instruction, that is affiliated with a <code>Position</code> on a
	 * document. The offset of the document event is not stored statically, but
	 * taken from the affiliated <code>Position</code>, which gets updated when
	 * other edits occur.
	 */
	static class Edit extends DocumentEvent {
		...
		public static class EditFactory {
			 ...
			 * position updater. <code>Edit</code>s managed by this factory are
			 * not updated after this call.
			 */
			public void release() {
				if (fDocument != null
	...
	protected String getCommentStart() {
		...
		return CommentsRule.START_COMMENTS;
	}
	...
	protected String getCommentEnd() {
		// for now: no space story
		return CommentsRule.END_COMMENTS;
	}
