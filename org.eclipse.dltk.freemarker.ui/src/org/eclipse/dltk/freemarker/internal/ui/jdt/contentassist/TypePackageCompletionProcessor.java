...
public abstract class TypePackageCompletionProcessor implements IContentAssistProcessor {
		...
		class TypePackageCompletionRequestor extends CompletionRequestor {
			...
			public void accept(CompletionProposal proposal) {
				if (proposal.getKind() == CompletionProposal.PACKAGE_REF) {
					...
					addProposalToCollection(c, startOffset, length, pkgName, pkgName, FreemarkerUIPluginImages.get(FreemarkerUIPluginImages.OBJ_DESC_PACKAGE));
				} else {
					...
					Image image = isInterface ? FreemarkerUIPluginImages.get(FreemarkerUIPluginImages.OBJ_DESC_GENERATE_INTERFACE) : FreemarkerUIPluginImages.get(FreemarkerUIPluginImages.OBJ_DESC_GENERATE_CLASS);
	...
	protected void generateTypeProposals(String currentContent, IProject project, final Collection c, final int startOffset, final int length, int typeScope) {
		...
		IJavaSearchScope scope = FreemarkerJavaHelper.getSearchScope(project);
		...
		try {
			TypeNameRequestor req = new TypeNameRequestor() {
				public void acceptType(int modifiers, char[] packageName, char[] simpleTypeName, char[][] enclosingTypeNames, String path) {
					...
					Image image = (Flags.isInterface(modifiers)) ? FreemarkerUIPluginImages.get(FreemarkerUIPluginImages.OBJ_DESC_GENERATE_INTERFACE) : FreemarkerUIPluginImages.get(FreemarkerUIPluginImages.OBJ_DESC_GENERATE_CLASS);
