...
public abstract class TableSection<T extends FreemarkerFormPage> extends StructuredViewerSection<T> {
	...
	class PartAdapter extends EditableTablePart {
		...
		protected void createButtons(Composite parent, FormToolkit toolkit) {
			...
			if (createCount()) {
				...
				fCount.setForeground(toolkit.getColors().getColor(
						IFormColors.TITLE));
				fCount.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));
				getTablePart().getTableViewer().getTable().addPaintListener(
						new PaintListener() {
		...
		protected void updateLabel() {
			if (fCount != null && !fCount.isDisposed())
				fCount.setText(NLS.bind(
						FreemarkerUIPluginMessages.TableSection_itemCount,
						Integer.toString(getTableViewer().getTable()
								.getItemCount())));
		}
	}
	 ...
	 * @param formPage
	 */
	public TableSection(T formPage, Composite parent,
