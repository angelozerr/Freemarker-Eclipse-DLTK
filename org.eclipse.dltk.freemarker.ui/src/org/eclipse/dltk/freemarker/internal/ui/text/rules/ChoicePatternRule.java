...
/**
 * Base class from {@link PatternRule} modified to manage start/end sequence with character choice (ex to detexct FTL Tag wich can start with '<' or '[').
 *
 */
public abstract class ChoicePatternRule implements IPredicateRule {
	...
	// protected TagSyntaxProvider tagSyntaxProvider;
	 ...
	 *            pattern
	 */
	public ChoicePatternRule(String tagName, IToken token,
			char escapeCharacter, boolean breaksOnEOL) {
		Assert.isTrue(tagName != null && tagName.length() > 0);
	...
	public ChoicePatternRule(String tagName, IToken token,
		...
		this(tagName, token, escapeCharacter, breaksOnEOL);
	...
	public ChoicePatternRule(String tagName, IToken token,
		...
		this(tagName, token, escapeCharacter, breaksOnEOL, breaksOnEOF);
	...
	protected abstract boolean isStartSequence(int c);
	...
	protected boolean endSequenceDetected(ICharacterScanner scanner) {
		...
		if (fLineDelimiters == null || fLineDelimiters.length != count) {
			...
			while (count > 0
					&& Arrays.equals(fLineDelimiters[count - 1],
		...
		int readCount = 1;
		int c;
		int bracketEnclosed = 0;
		boolean simpleQuoteEnclosed = false;
		boolean doubleQuoteEnclosed = false;
		while ((c = scanner.read()) != ICharacterScanner.EOF) {
			...
			case '(':
				...
				break;
			case ')':
				if (bracketEnclosed > 0) {
					bracketEnclosed--;
				}
				break;
			case '\'':
				simpleQuoteEnclosed = !simpleQuoteEnclosed;
				break;
			case '\"':
				doubleQuoteEnclosed = !doubleQuoteEnclosed;
				break;
			}
			...
			} else if (!isEnclosed(bracketEnclosed, simpleQuoteEnclosed,
					doubleQuoteEnclosed) && isEndSequence(c)) {
	...
	protected abstract boolean isEndSequence(int c);

	private boolean isEnclosed(int bracketEnclosed,
			boolean simpleQuoteEnclosed, boolean doubleQuoteEnclosed) {
		if (bracketEnclosed > 0) {
			return true;
		}
		if (simpleQuoteEnclosed) {
			return true;
		}
		if (doubleQuoteEnclosed) {
			return true;
		}
		return false;
	}
